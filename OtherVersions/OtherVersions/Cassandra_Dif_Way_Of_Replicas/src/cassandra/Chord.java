...
{    
    ...
    public Chord()
    {
        ...
        for (int i = 0; i < number_of_chord_nodes; i++) {
            ...
            int nodes_index = getHash(nodes.get(i));
    ...
    // our hash function
    // we ahould have returned unsigned int or long
    // in order to be safe in very big numbers
    // but we didn't have time to change it and we thought
    ...
    public int getHash(String s)  
    {
        ...
        try {
            ...
            md.update(s.getBytes("UTF-8")); 
               ...
               num = Math.abs( num % (int) Math.pow(2, m));
    ...
    { 
        Map<Integer,String> keysNvalue = new LinkedHashMap<>();
        ...
        String copy = null;
        Chord_node node, temp_chord_node, last_chord_node, temp_chord_node2;    
        ...
        List<Map.Entry<Integer, String>> entries = new ArrayList<>(keysNvalue.entrySet());
        ...
        */
        ...
        Collections.sort(entries, (Map.Entry< Integer, String> a, Map.Entry<Integer, String> b) -> a.getKey().compareTo(b.getKey()));
        ...
        for (Integer key: keys) {
            ...
            // put half of replicas in node's successors and half of them in predeccessors
            // if no of replicas is an odd number add 1
            temp_chord_node2 = temp_chord_node;
            int it = no_of_replicas/2;
            if (no_of_replicas%2 != 0 && it!=0) it+=1;
            for (int j = 0; j < it; j++) {
                temp_chord_node = temp_chord_node.successor;
                temp_chord_node.key.add(key);
                temp_chord_node.data.add(sortedMap.get(key));
            ...
            for(int j=0; j<no_of_replicas-it; j++) {
                temp_chord_node2 = temp_chord_node2.predecessor;
                temp_chord_node2.key.add(key);
                temp_chord_node2.data.add(sortedMap.get(key));
                
                dates = null;
                for (String line : temp_chord_node2.data) { 
                    dates = line.split(",");
                    break;
                }
                
                temp_chord_node2.data_date = data_date(dates[11].replaceAll("\"",""));
    ...
    // procedure needed
    public int searchInChord(int hash_key, boolean searchVersion)
    {        
    ...
    private Chord_node findLastVersionNode(Chord_node node, boolean searchVersion, int hash_key)
    {
    ...
    protected class Chord_node 
    {
        ...
        ArrayList<Integer> key;                 // keys that node is responsible for
        ArrayList<String> data;                 // data that each key has
        int node_index;                         // index of this node [0 to (2^m-1)]
        public String node_name;                // Name of node, e.g., N35   
        public double data_date;                // date of key to define version

        // Initialize everything
        public Chord_node(int node_index)
        {            
            this.node_index = node_index;
            this.node_name = "N" + Integer.toString(node_index);
